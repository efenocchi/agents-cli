 ...
 * orgs (npm: @swarmify; github: phnx-labs). Every reference must use the
 * correct one, or `agents upgrade` installs the wrong package, version-check
 * silently 404s, or the landing page misleads users.
 *
 * Caught in audit RUSH-595 — fix-install over-normalized to swarmify, leaving
 * three broken `@phnx-labs/agents-cli@latest` references in src/index.ts and
 * an invalid github.com/swarmify/agents-cli repository URL in package.json.
 */
...
const NPM_PACKAGE = '@swarmify/agents-cli';
...
describe('package-name dual identity (npm vs github)', () => {
  ...
  it('src/index.ts never references the wrong @phnx-labs npm scope', () => {
    ...
    // The npm scope is @swarmify. Any @phnx-labs/agents-cli substring is a bug.
