  ...
  try {
    ...
    if (latestVersion !== VERSION && compareVersions(latestVersion, VERSION) > 0) {
      ...
      if (answer === 'now') {
        ...
        try {
          await execAsync('npm install -g @companion/agents-cli@latest');
...
program
  ...
  .action(async () => {
    ...
    try {
      ...
      try {
        const npmList = execSync('npm list -g @companion/agents-cli 2>/dev/null', { encoding: 'utf-8' });
        if (npmList.includes('@companion/agents-cli')) {
          cmd = 'npm install -g @companion/agents-cli@latest';
        } else {
        ...
        try {
          ...
          if (bunList.includes('@companion/agents-cli')) {
            cmd = 'bun install -g @companion/agents-cli@latest';
          } else {
      ...
      spinner.fail('Upgrade failed');
      console.error(chalk.red((err as Error).message));
      console.log(chalk.gray('\nManual upgrade: npm install -g @companion/agents-cli@latest'));
      process.exit(1);
    }
  });
