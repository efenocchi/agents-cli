 ...
 *         (a few stat calls). Node writes the sentinel after each
 *         successful sync. Documented limitation: POSIX dir mtime only
 *         updates on top-level entry add/remove, so deep edits to plugin
 *         contents won't trigger auto-resync — `agents sync` for that.
 */
