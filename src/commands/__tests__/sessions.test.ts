...
describe('agents sessions', () => {
  it('lists only sessions from the current directory by default and shows all with --all', () => {
    ...
    try {
      ...
      const companionDir = path.join(tempHome, 'work', 'companion');
      const agentsCliDir = path.join(tempHome, 'work', 'agents-cli');
      const companionSessionId = '11111111-1111-4111-8111-111111111111';
      ...
      writeClaudeSession(
        ...
        companionSessionId,
        companionDir,
        'Inspect the companion session list',
      ...
      const localResult = runAgents(['sessions'], companionDir, tempHome);
      expect(localResult.status).toBe(0);

      const localOutput = outputOf(localResult);
      expect(localOutput).toContain(companionSessionId.slice(0, 8));
      expect(localOutput).not.toContain(agentsCliSessionId.slice(0, 8));

      const allResult = runAgents(['sessions', '--all'], companionDir, tempHome);
      expect(allResult.status).toBe(0);

      const allOutput = outputOf(allResult);
      expect(allOutput).toContain(companionSessionId.slice(0, 8));
  ...
  it('finds matching projects outside the current directory when --project is provided', () => {
    ...
    try {
      ...
      const companionSessionId = '55555555-5555-4555-8555-555555555555';
      ...
      expect(output).not.toContain(companionSessionId.slice(0, 8));
...
describe('agents sessions view', () => {
  it('resolves explicit IDs across directories even when the default listing is scoped to cwd', () => {
    ...
    try {
      ...
      const result = runAgents(['sessions', 'view', siblingSessionId, '--transcript'], companionDir, tempHome);
      expect(result.status).toBe(0);
  ...
  it('resolves Claude /resume history IDs to the resumed transcript', () => {
    ...
    try {
      ...
      const projectRoot = path.join(tempHome, 'work', 'companion');
      ...
      fs.mkdirSync(path.join(tempHome, '.claude', 'projects', 'companion-test'), { recursive: true });
      fs.writeFileSync(
      ...
      fs.writeFileSync(
        path.join(tempHome, '.claude', 'projects', 'companion-test', `${transcriptId}.jsonl`),
        [
          JSON.stringify({
            type: 'user',
            timestamp: '2026-04-17T19:00:00.000Z',
